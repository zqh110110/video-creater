...
exports.TianmuMCPServer = void 0;
 ...
 * 天幕MCP服务器
 */
class TianmuMCPServer {
    constructor(config) {
        this.client = new tianmu_client_1.TianmuClient(config);
    ...
    async handleContinueVideo(args) {
    ...
    async handleFramesToVideo(args) {
    ...
    async handleTextToSoundEffect(args) {
    ...
    async handleGenerateVideoSoundtrack(args) {
    ...
    async handleRedrawingImage(args) {
    ...
    async handleRecognizeImage(args) {
    ...
    async handleWaitForTask(args) {
        const { task_id, max_attempts, interval_ms } = args;
        try {
            const result = await this.client.waitForTaskCompletion(task_id, max_attempts || 40, // 默认40次
            ...
            if (result.images_path && result.images_path.length > 0) {
                outputText += '📷 图片URL:\n';
                result.images_path.forEach((url, i) => {
                    outputText += `${i + 1}. ${url}\n`;
                });
            }
            else if (result.video_path && result.video_path.length > 0) {
                outputText += '🎬 视频URL:\n';
                result.video_path.forEach((url, i) => {
                    outputText += `${i + 1}. ${url}\n`;
                });
            }
...
exports.TianmuMCPServer = TianmuMCPServer;
